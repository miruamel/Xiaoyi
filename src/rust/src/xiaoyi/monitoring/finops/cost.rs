...
/// @return `Result<CostEstimate>` New cost estimate or error if pricing not found
    ...
    /// @since 0.1.0
    /// @author Miruamel
    pub fn estimate(model: &str, prompt_tokens: u64, completion_tokens: u64) -> Result<CostEstimate> {
        ...
        let price_per_1k_tokens = match model {
            ...
            "llama2-70b" => 0.0015,
            "mistral-7b" => 0.001,
            _ => return Err(XiaoyiError::new(ErrorKind::Llm, format!("Unknown model pricing: {}", model))),
        };
